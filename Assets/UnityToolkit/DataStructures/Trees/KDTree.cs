...
namespace ProjectWorlds.DataStructures.Trees
{
    ...
    public class KDTree<T> : System.Collections.Generic.IEnumerable<System.Collections.Generic.KeyValuePair<double[], T>>, System.Collections.IEnumerable
    {
        ...
        protected class KDTreeEnumerator : System.Collections.Generic.IEnumerator<System.Collections.Generic.KeyValuePair<double[], T>>, System.Collections.IEnumerator
        {
            ...
            private ProjectWorlds.DataStructures.Queues.Queue<System.Collections.Generic.KeyValuePair<double[], T>> queue;

            public KDTreeEnumerator(KDTree<T> tree)
            {
                this.tree = tree;
                queue = new ProjectWorlds.DataStructures.Queues.Queue<System.Collections.Generic.KeyValuePair<double[], T>>();
        ...
        public OrderedList<float, T> GetNearestNeighbors(double[] point, int maxItems)
        {
            ...
            ProjectWorlds.DataStructures.Queues.Queue<Node> toSearch = new ProjectWorlds.DataStructures.Queues.Queue<Node>() { head };
        ...
        public OrderedList<float, T> RadialSearch(double[] center, float dist)
        {
            if (dist < 0) {
            ...
            while (toSearch.Count > 0)
            {
                ...
                if (curNode.IsLeaf)
                {
                    ...
                    if (CircleIntersectsRect(center, dist, curNode.maxBound, curNode.minBound))
                    {
                        for (int i = 0; i < curNode.count; i++)
                        {
                            float distance = Distance(items[(curNode.nodeIndex * maxNodeItems) + i].Key, center);
                            if (distance <= dist)
                            {
                                result.Add(distance, items[(curNode.nodeIndex * maxNodeItems) + i].Value);
                            }
                        }
                    }
                }
