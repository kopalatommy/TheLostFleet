...
namespace GalacticBoundStudios.HexTech.PathFinding
{
    ...
    public partial struct HexTechPathfinderSystem : ISystem
    {
        ...
        void OnUpdate(ref SystemState state)
        {
            ...
            // if (requestEntities.Length > 0)
            //     {
            //         NativeArray<HexTechCreatePathRequest> requestArray = new NativeArray<HexTechCreatePathRequest>(requestEntities.Length, Allocator.TempJob);
            //         NativeArray<NativeList<HexCoord>> resultingPaths = new NativeArray<NativeList<HexCoord>>(requestEntities.Length, Allocator.TempJob);
            //         for (int i = 0; i < requestEntities.Length; i++)
            //         {
            //             requestArray[i] = state.EntityManager.GetComponentData<HexTechCreatePathRequest>(requestEntities[i]);
            //             resultingPaths[i] = new NativeList<HexCoord>(Allocator.Persistent);

            //             // Add component so that it is not grabbed again
            //             state.EntityManager.AddComponentData<HexTechPathfinderPathInProgressFlag>(requestEntities[i], new HexTechPathfinderPathInProgressFlag());
            //         }

            //         HexTechCreatePathJobParallel job = new HexTechCreatePathJobParallel
            //         {
            //             requestArray = requestArray,
            //             resultsArray = resultingPaths,
            //             requestEntities = requestEntities,
            //         };

            //         JobHandle jobHandle = job.Schedule(requestArray.Length, 1);
            ...
            // {
            //     for (int index = 0; index < activeJobs.Length; index++)
            //     {
            //         if (activeJobs[index].jobHandle.IsCompleted)
            //         {
            //             // Must call complete before deallocating any job data even if complete.
            //             activeJobs[index].jobHandle.Complete();

            //             NativeArray<HexTechCreatePathRequest> _requestArray = activeJobs[index].job.requestArray;
            //             NativeArray<NativeList<HexCoord>> _resultsArray = activeJobs[index].job.resultsArray;
            //             NativeArray<Entity> _requestEntities = activeJobs[index].job.requestEntities;

            //             for (int j = 0; j < _requestArray.Length; j++)
            //             {
            //                 if (state.EntityManager.HasComponent<HexTechMapPath>(_requestEntities[j]))
            //                 {
            //                     state.EntityManager.RemoveComponent<HexTechMapPath>(_requestEntities[j]);
            //                 }

            //                 if (_resultsArray[j].IsEmpty)
            //                 {
            //                     Debug.Log("Received empty path");

            //                     _resultsArray[j].Dispose();
            //                     continue;
            //                 }

            //                 state.EntityManager.AddComponentData(_requestEntities[j], new HexTechMapPath
            //                 {
            //                     path = _resultsArray[j]
            //                 });

            //                 string path = "(Start) ";
            //                 for (int k = 0; k < _resultsArray[j].Length; k++)
            //                 {
            //                     path += _resultsArray[j][k] + " -> ";
            //                 }
            //                 path += "(end)";
            //                 Debug.Log(path);
            //             }
